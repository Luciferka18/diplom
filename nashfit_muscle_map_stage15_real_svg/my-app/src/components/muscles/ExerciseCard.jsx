...
export default function ExerciseCard({ exercise }) {
  ...
  return (
    <article className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-4 transition hover:-translate-y-0.5 hover:border-[#ff4d4f]/50">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h4 className="mt-3 text-lg font-black tracking-[-0.02em] text-[color:var(--text)]">{exercise.name}</h4>
          {exercise.description ? (
            <p className="mt-2 line-clamp-2 text-sm leading-6 text-[color:var(--muted)]">{exercise.description}</p>
          ) : null}
        </div>
        {exercise.video_url ? (
          <a
            href={exercise.video_url}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#ff4d4f] text-white"
            aria-label={`Открыть видео: ${exercise.name}`}
          >
            <PlayCircle size={20} />
          </a>
        ) : (
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[color:var(--accentGlow)] text-[color:var(--accent)]">
            <Dumbbell size={20} />
          </div>
        )}
      </div>
