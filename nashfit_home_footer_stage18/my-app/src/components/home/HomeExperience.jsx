...
function TrainerCard({ item, featured = false }) {
  return (
    <Link
      ...
      className={`group flex h-full flex-col rounded-[1.7rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-4 transition hover:-translate-y-1 hover:border-[color:var(--accent)]/45 hover:shadow-xl hover:shadow-emerald-950/10 dark:hover:shadow-black/30 ${featured ? "lg:p-5" : ""}`}
    >
      <div className={`grid gap-4 ${featured ? "md:grid-cols-[180px_1fr]" : "grid-cols-[96px_1fr]"}`}>
        <Media
          src={item.photo_url}
          alt={item.name}
          fallback="/demo/trainers/anna-volkova.svg"
          className={`${featured ? "aspect-[4/4.6] md:h-full" : "h-28 w-24"} rounded-2xl border border-[color:var(--stroke)]`}
        />
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {Number(item.avg_rating) > 0 ? (
              <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/25 bg-amber-400/10 px-2.5 py-1 text-xs font-black text-amber-600 dark:text-amber-300">
                <Star size={13} fill="currentColor" /> {item.avg_rating}
              </span>
            ) : null}
          </div>
          <h3 className={`${featured ? "mt-4 text-3xl" : "mt-3 text-xl"} line-clamp-2 font-black tracking-[-0.03em] text-[color:var(--text)] group-hover:text-[color:var(--accent)]`}>{item.name}</h3>
          <p className="mt-2 line-clamp-2 text-sm leading-6 text-[color:var(--muted)]">{item.specialization || item.bio || "Персональные тренировки, техника и понятный план прогресса."}</p>
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-[color:var(--muted)]">
            <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">{item.experience_years || 1} лет опыта</span>
            <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">{item.location || item.club || "НашФит"}</span>
          </div>
        </div>
      </div>
      <div className="mt-4 flex flex-1 flex-col justify-between gap-4 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4">
        <div className="text-sm leading-6 text-[color:var(--muted)]">
          Подойдёт, если нужна техника, дисциплина и индивидуальная корректировка программы под цель.
        </div>
        <div className="inline-flex items-center justify-between gap-3 font-black text-[color:var(--text)]">
...
function ProductCard({ item, featured = false }) {
  return (
    <Link
      href={`/shop/${item.id}`}
      className={`group overflow-hidden rounded-[1.7rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-3 transition hover:-translate-y-1 hover:border-[color:var(--accent)]/45 hover:shadow-xl hover:shadow-emerald-950/10 dark:hover:shadow-black/30 ${featured ? "grid gap-4 md:grid-cols-[260px_1fr] md:p-4" : ""}`}
    >
      <Media src={item.image_url} alt={item.name} fallback="/demo/products/equipment.svg" className={`${featured ? "aspect-[4/3] md:h-full" : "aspect-square"} rounded-[1.25rem] border border-[color:var(--stroke)]`} />
      <div className="flex flex-col p-2 pt-4 md:p-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-[color:var(--accentGlow)] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.13em] text-[color:var(--accent)]">
            {item.category || item.brand || "НашФит Store"}
          </span>
          {item.old_price ? <span className="rounded-full bg-amber-400/12 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.13em] text-amber-600 dark:text-amber-300">Скидка</span> : null}
        </div>
        <h3 className={`${featured ? "mt-4 text-3xl" : "mt-3 text-lg"} line-clamp-2 font-black tracking-[-0.03em] text-[color:var(--text)] group-hover:text-[color:var(--accent)]`}>{item.name || item.title}</h3>
        <p className={`${featured ? "line-clamp-4" : "line-clamp-2"} mt-2 text-sm leading-6 text-[color:var(--muted)]`}>
          {item.short_description || item.description || "Подходит для домашних тренировок, восстановления и прогресса по программам НашФит."}
        </p>
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[color:var(--muted)]">
          <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">Для программы</span>
          <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">В наличии</span>
        </div>
        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
          <div>
            <div className="text-2xl font-black text-[color:var(--text)]">{displayPrice(item.price)}</div>
            {item.old_price ? <div className="text-xs text-[color:var(--muted)] line-through">{displayPrice(item.old_price)}</div> : null}
          </div>
          <div className="inline-flex items-center gap-2 rounded-xl bg-[color:var(--accent)] px-3.5 py-3 text-sm font-black text-white">
            <ShoppingBag size={17} /> {featured ? "Открыть" : ""}
          </div>
        </div>
      </div>
    </Link>
...
function ArticleCard({ item, featured = false }) {
  return (
    <Link href={`/articles/${item.id}`} className={`group overflow-hidden rounded-[1.7rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] transition hover:-translate-y-1 hover:border-[color:var(--accent)]/45 hover:shadow-xl hover:shadow-emerald-950/10 dark:hover:shadow-black/30 ${featured ? "grid md:grid-cols-[1.05fr_.95fr]" : ""}`}>
      <Media src={item.cover_image_url || item.image_url} alt={item.title} fallback="/demo/covers/nutrition.svg" className={featured ? "min-h-[320px]" : "aspect-[16/10]"} />
      <div className="flex flex-col p-5 md:p-6">
        ...
        <h3 className={`${featured ? "text-3xl md:text-4xl" : "text-xl"} mt-4 font-black tracking-[-0.03em] text-[color:var(--text)] group-hover:text-[color:var(--accent)]`}>{item.title}</h3>
        <p className="mt-3 line-clamp-3 text-sm leading-6 text-[color:var(--muted)]">{item.excerpt || item.description}</p>
        <div className="mt-5 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 text-sm leading-6 text-[color:var(--muted)]">
          Короткий практический материал: что делать, зачем это нужно и как применить в тренировках.
        </div>
        <div className="mt-auto inline-flex items-center gap-2 pt-5 font-black text-[color:var(--accent)]">Читать статью <ArrowRight size={17} /></div>
      </div>
    </Link>
  );
}
    ...
    <main className="min-h-screen bg-[color:var(--bg)] text-[color:var(--text)]">
      ...
      <section className="bg-[color:var(--panel)] px-4 py-14 md:px-8 md:py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-7 grid gap-4 rounded-[1.7rem] border border-[color:var(--accent)]/40 bg-[linear-gradient(120deg,rgba(34,183,143,.13),transparent)] p-6 md:grid-cols-[1fr_auto] md:items-center">
            <div>
