...
function AboutGyms({ gym, locations = [] }) {
  ...
  return (
    <section id="clubs" className="px-4 py-9 md:px-8 md:py-12">
...
export default function HomeExperience({ data = {} }) {
  ...
  return (
    <main className="min-h-screen bg-[color:var(--bg)] text-[color:var(--text)]">
      ...
      <section className="border-y border-[color:var(--stroke)] bg-[color:var(--panel)] px-4 py-7 md:px-8 md:py-9">
        <div className="mx-auto max-w-7xl">
          <div className="rounded-[1.8rem] border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 shadow-[0_14px_44px_rgba(0,0,0,.045)] md:p-5">
            <div className="grid gap-4 lg:grid-cols-[.9fr_1fr_1fr_1fr] lg:items-stretch">
              <div className="rounded-[1.35rem] bg-[linear-gradient(135deg,rgba(47,113,82,.10),rgba(47,113,82,.03))] p-5">
                <div className="mb-2 inline-flex rounded-full bg-[color:var(--accentGlow)] px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-[color:var(--accent)]">
                  Как это работает
                </div>
                <h2 className="text-2xl font-black leading-tight tracking-[-0.03em] text-[color:var(--text)]">
                  3 шага до нормального старта
                </h2>
                <p className="mt-3 text-sm leading-6 text-[color:var(--muted)]">
                  Сначала выбираем цель, затем даём понятный план и доводим до тренировки в клубе.
                </p>
              </div>

              {[
                { icon: Target, title: "Выберите цель", text: "Программа, мышечная зона, статьи и товары под ваш уровень.", meta: "персональный старт" },
                { icon: HeartPulse, title: "Тренируйтесь регулярно", text: "Возвращайтесь к упражнениям, отмечайте прогресс и держите ритм.", meta: "контроль прогресса" },
                { icon: Route, title: "Приходите в зал", text: "Адрес, график, карта и запись к тренеру — рядом с планом.", meta: "офлайн-поддержка" },
              ].map((item, index) => (
                <div
                  key={item.title}
                  className="relative min-h-[150px] overflow-hidden rounded-[1.35rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 transition hover:-translate-y-0.5 hover:border-[color:var(--accent)]/45"
                >
                  <div className="absolute right-4 top-3 text-3xl font-black tracking-[-0.07em] text-[color:var(--accent)]/18">0{index + 1}</div>
                  <div className="relative flex h-full flex-col">
                    <div className="mb-4 flex items-center gap-3">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[color:var(--accentGlow)] text-[color:var(--accent)]">
                        <item.icon size={23} />
                      </div>
                      <div className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[color:var(--muted)]">
                        {item.meta}
                      </div>
                    </div>
                    <h3 className="text-lg font-black tracking-[-0.02em] text-[color:var(--text)]">{item.title}</h3>
      ...
      <section className="bg-[color:var(--panel)] px-4 py-8 md:px-8 md:py-11">
        <div className="mx-auto max-w-7xl">
