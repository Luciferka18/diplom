...
class NashfitDemoContentSeeder extends Seeder
{
    ...
    {
        $plans = [
            ['slug'=>'1-month','name'=>'1 месяц','duration_months'=>1,'price'=>390000,'old_price'=>null,'badge'=>'Гибкий старт','featured'=>false],
            ['slug'=>'3-months','name'=>'3 месяца','duration_months'=>3,'price'=>990000,'old_price'=>1170000,'badge'=>'Выгодно','featured'=>true],
            ['slug'=>'6-months','name'=>'6 месяцев','duration_months'=>6,'price'=>1690000,'old_price'=>2340000,'badge'=>'Лучший выбор','featured'=>true],
            ['slug'=>'12-months','name'=>'1 год','duration_months'=>12,'price'=>2790000,'old_price'=>4680000,'badge'=>'Максимум пользы','featured'=>false],
            ['slug'=>'24-months','name'=>'2 года','duration_months'=>24,'price'=>4590000,'old_price'=>9360000,'badge'=>'Для серьёзных целей','featured'=>false],
            ['slug'=>'trial-3-visits','name'=>'3 бесплатных посещения','duration_months'=>null,'price'=>0,'old_price'=>null,'badge'=>'Для новых гостей','featured'=>false,'trial'=>true],
        ];
        $result = [];
        foreach ($plans as $sort => $item) {
            $model = $this->upsert(Membership::class, ['slug'=>$item['slug']], [
                'name'=>$item['name'],'description'=>'Доступ в зал НашФит, групповые зоны и поддержку команды.',
                'duration_months'=>$item['duration_months'],'trial_visits'=>$item['trial'] ?? false ? 3 : 0,'price'=>$item['price'],'old_price'=>$item['old_price'],
                'features'=>['Современный тренажёрный зал','Раздевалки и душевые','Бесплатные программы в приложении','Поддержка тренеров'],
                'badge'=>$item['badge'],'is_trial'=>$item['trial'] ?? false,'is_featured'=>$item['featured'],'is_active'=>true,'sort_order'=>$sort,
            ]);
            $result[$item['slug']] = $model;
        }
        $promo = $this->upsert(Promotion::class, ['slug'=>'demo-season'], [
            'name'=>'Сезон сильных привычек','description'=>'Скидка на покупки и услуги в период демонстрации.','discount_type'=>'percent','discount_value'=>12,
            'applies_to'=>['shop','membership','booking'],'auto_apply'=>false,'is_active'=>true,'starts_at'=>now()->subMonth(),'ends_at'=>now()->addMonths(3),
            'badge'=>'-12%','banner_title'=>'Начните сейчас','banner_text'=>'Примените промокод FIT12 к магазину, абонементу или тренировке.',
        ]);
        $this->upsert(PromoCode::class, ['code'=>'FIT12'], [
            'promotion_id'=>$promo->id,'description'=>'Демо-промокод на все услуги','discount_type'=>'percent','discount_value'=>12,
            'applies_to'=>['shop','membership','booking'],'minimum_amount'=>0,'max_uses'=>500,'per_user_limit'=>2,'uses_count'=>0,
        ...
        foreach ($trainerList as $i=>$trainer) {
            foreach (array_slice($productList,$i%8,3) as $product) {
                DB::table('trainer_product_recommendations')->updateOrInsert(
                    ...
                    ['comment'=>'Использую в работе и рекомендую для безопасного прогресса.','is_featured'=>true,'created_at'=>now(),'updated_at'=>now()]
                );
            }
        }
    ...
    private function seedShopCollectionsAndRelations(array $products): void
    {
        ...
        $pivotHasCreatedAt = Schema::hasColumn('shop_collection_product', 'created_at');
        $pivotHasUpdatedAt = Schema::hasColumn('shop_collection_product', 'updated_at');
        ...
        foreach ($sets as $sort => $set) {
            ...
            foreach ($set['items'] as $itemSort => $slug) {
                ...
                $pivotData = ['sort_order' => $itemSort];
                if ($pivotHasCreatedAt) $pivotData['created_at'] = now();
                if ($pivotHasUpdatedAt) $pivotData['updated_at'] = now();
        ...
        foreach ($productList as $index => $product) {
            foreach ([$productList[($index + 1) % count($productList)] ?? null, $productList[($index + 5) % count($productList)] ?? null] as $relationIndex => $related) {
                ...
                DB::table('product_relations')->updateOrInsert(
                    ...
                    ['type' => $relationIndex === 0 ? 'also_bought' : 'recommended', 'sort_order' => $relationIndex, 'created_at' => now(), 'updated_at' => now()]
                );
            }
        }
    }
