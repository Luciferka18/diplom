...
class ArticleController extends Controller
{
    ...
    public function moderate(Request $request, Article $article)
    {
        ...
        $statusTitle = match ($article->status) {
            'published' => 'Статья опубликована',
            'rejected' => 'Статья отклонена',
            'archived' => 'Статья скрыта',
            default => 'Статус статьи изменён',
        };
        $statusBody = $article->status === 'rejected'
            ? ($article->rejection_reason ?: 'Администратор вернул статью на доработку.')
            : 'Материал «' . $article->title . '» получил новый статус.';
        app(ActivityService::class)->notifyUser($article->author_user_id, 'article.moderated', $statusTitle, $statusBody, '/account/articles', $article, ['article_id' => $article->id, 'status' => $article->status], $request->user()->id, 'newspaper');
