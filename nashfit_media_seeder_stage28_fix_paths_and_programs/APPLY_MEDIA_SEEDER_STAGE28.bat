...
if not exist "%PROJECT_ROOT%\my-app" (
  ...
  echo Распакуйте nashfit_media_seeder_stage28_fix_paths_and_programs в корень проекта, где лежат my-app и fitlab.
  pause
  exit /b 1
)
...
echo.
echo [OK] Media Seeder Stage28 установлен.
echo.
echo Что появилось:
echo - WebP-картинки продублированы и в fitlab/public/seed-images, и в my-app/public/seed-images
echo - фото тренеров в TrainerSeeder
echo - обложки статей в ArticleSeeder
echo - картинки, галереи и варианты товаров в ProductSeeder
echo - картинки программ в ProgramSeeder
echo.
