...
function SummaryStrip({ loading, count, groups, bestResult, cleanQuery }) {
  const activeGroups = groups.filter((group) => group.items.length);
  const breakdown = activeGroups.length
    ? activeGroups.map((group) => `${group.title}: ${group.items.length}`).join(" · ")
    : "Появится после ввода запроса";

  return (
    <section className="container-fitlab mt-6">
      <div className="grid gap-4 lg:grid-cols-[1fr_1.2fr_1.4fr]">
        <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5">
          ...
          <div className="text-sm text-[color:var(--muted)]">{cleanQuery ? `по запросу «${cleanQuery}»` : "результатов сейчас"}</div>
        </div>

        <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5">
          <Lightbulb className="h-6 w-6 text-cyan-600 dark:text-cyan-300" />
          <div className="mt-3 line-clamp-1 text-xl font-black text-[color:var(--text)]">
            {bestResult?.title || (loading ? "Ищем совпадение" : "Лучшее совпадение")}
          </div>
          <div className="text-sm text-[color:var(--muted)]">{bestResult?.label || "самый релевантный результат"}</div>
        </div>

        <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5">
          <Layers3 className="h-6 w-6 text-amber-600 dark:text-amber-300" />
          <div className="mt-3 text-xl font-black text-[color:var(--text)]">Разделы с результатами</div>
          <div className="mt-1 line-clamp-2 text-sm text-[color:var(--muted)]">{breakdown}</div>
        </div>
      </div>
    </section>
  );
...
export default function SearchPage() {
  ...
  return (
    <main className="min-h-screen bg-[color:var(--bg)] pb-20">
      <section className="container-fitlab pt-8">
        <div className="relative overflow-hidden rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6 md:p-10">
          ...
          <div className="relative max-w-4xl">
            ...
            <div className="mt-5">
              <div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-[color:var(--muted)]">
      ...
      <SummaryStrip loading={loading} count={count} groups={groups} bestResult={bestResult} cleanQuery={cleanQuery} />
