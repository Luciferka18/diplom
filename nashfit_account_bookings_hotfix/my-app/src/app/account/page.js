...
function normalizeSingle(response, fallbackKey) {
  if (!response) return null;
  if (response?.data && !Array.isArray(response.data)) return response.data;
  if (response?.[fallbackKey]) return response[fallbackKey];
  return null;
}

async function loadFirstList(endpoints, fallbackKey) {
  for (const endpoint of endpoints) {
    try {
      const response = await apiGet(endpoint);
      const list = normalizeList(response, fallbackKey);
      if (list.length || Array.isArray(response) || Array.isArray(response?.data) || Array.isArray(response?.data?.data) || Array.isArray(response?.[fallbackKey])) {
        return { ok: true, items: list };
      }

      const single = normalizeSingle(response, fallbackKey.replace(/s$/, ""));
      if (single) return { ok: true, items: [single] };
    } catch (error) {
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const [ordersResult, bookingsResult, reviewsResult, notificationsResponse, programsResponse] = await Promise.all([
        loadFirstList(["/orders?per_page=20", "/orders"], "orders"),
        loadFirstList(["/bookings?per_page=20", "/account/bookings?per_page=20", "/bookings", "/bookings/next"], "bookings"),
        loadFirstList(["/account/reviews?per_page=20", "/reviews?per_page=20", "/reviews"], "reviews"),
        Promise.resolve(apiGet("/account/notifications?per_page=8")).then((value) => ({ status: "fulfilled", value })).catch((reason) => ({ status: "rejected", reason })),
        Promise.resolve(apiGet("/account/programs")).then((value) => ({ status: "fulfilled", value })).catch((reason) => ({ status: "rejected", reason })),
      ]);
      ...
      const allReviews = reviewsResult.items;
      ...
      const accountCompletelyUnavailable = !ordersResult.ok && !bookingsResult.ok && !reviewsResult.ok && notificationsResponse.status === "rejected" && programsResponse.status === "rejected";
      if (accountCompletelyUnavailable) {
        setError("Данные кабинета не загрузились. Проверь backend Laravel и авторизацию.");
      }
      setLoading(false);
    }
