...
const PROGRAM_FALLBACKS = {
  strength: [
    {
      title: "Силовая база",
      description: "Базовая программа с приседом, жимом и тягой для развития силы и мышечной массы.",
      level: "intermediate",
      duration_weeks: 12,
      href: "/programs",
      match_reason: "связана с базовыми силовыми движениями",
    },
  ],
  power: [
    {
      title: "Мощь и сила",
      description: "Продвинутая программа для роста силовых показателей в основных движениях.",
      level: "advanced",
      duration_weeks: 16,
      href: "/programs",
      match_reason: "подходит для силового прогресса",
    },
  ],
  functional: [
    {
      title: "Функциональный старт",
      description: "Базовая программа для укрепления корпуса, контроля техники и подготовки всего тела.",
      level: "beginner",
      duration_weeks: 4,
      href: "/programs",
      match_reason: "связана с укреплением корпуса и общей подготовкой",
    },
  ],
  fatLoss: [
    {
      title: "Похудение за 8 недель",
      description: "Кардио и силовые упражнения для снижения веса и тонуса основных групп мышц.",
      level: "intermediate",
      duration_weeks: 8,
      href: "/programs",
      match_reason: "связана с общей силовой нагрузкой и тонусом",
    },
  ],
};

const MUSCLE_PROGRAMS = {
  chest: ["strength", "power"],
  shoulders: ["strength", "power"],
  biceps: ["strength"],
  triceps: ["strength", "power"],
  forearms: ["functional"],
  abs: ["functional", "fatLoss"],
  obliques: ["functional", "fatLoss"],
  lats: ["strength", "power"],
  traps: ["strength"],
  lower_back: ["functional", "strength"],
  glutes: ["strength", "fatLoss"],
  quads: ["strength", "power", "fatLoss"],
  hamstrings: ["strength"],
  calves: ["fatLoss", "strength"],
};

function fallbackProgramsFor(slug) {
  return (MUSCLE_PROGRAMS[slug] || [])
    .flatMap((key) => PROGRAM_FALLBACKS[key] || [])
    .slice(0, 2);
}
...
export function getFallbackMuscle(slug = "chest") {
  const muscle = MUSCLE_FALLBACK[slug] || MUSCLE_FALLBACK.chest;
  return {
    ...muscle,
    related_programs: muscle.related_programs || fallbackProgramsFor(muscle.slug),
  };
}
...
export function normalizeMuscle(raw, slug = "chest") {
  ...
  return {
    ...fallback,
    ...raw,
    related_programs: Array.isArray(raw.related_programs) ? raw.related_programs : fallback.related_programs || fallbackProgramsFor(fallback.slug),
