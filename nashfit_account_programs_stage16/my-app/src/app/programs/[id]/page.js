...
const spreadWorkoutDays = [1, 3, 5, 6];
const restTemplates = {
  2: {
    kind: "Восстановление",
    title: "Лёгкая активность и прогулка",
    duration: "20 мин",
    description: "Спокойная ходьба, суставная разминка и дыхание. Задача дня — восстановиться и не забивать мышцы после нагрузки.",
  },
  4: {
    kind: "Мобильность",
    title: "Растяжка и техника",
    duration: "25 мин",
    description: "Мягкая растяжка бёдер, грудного отдела и плеч. Можно повторить технику базовых упражнений без тяжёлого веса.",
  },
  6: {
    kind: "Поддержка",
    title: "Питание и спортивные добавки",
    duration: "10 мин",
    description: "Проверь воду, белок и восстановление. В этот день можно подобрать протеин, креатин или батончик под цель программы.",
    product: true,
  },
  7: {
    kind: "Отдых",
    title: "Полный отдых",
    duration: "—",
    description: "Без тяжёлых тренировок. Сон, прогулка, лёгкая мобилизация и подготовка к следующей неделе.",
  },
};

function buildWeeklySchedule(selectedWorkouts, products) {
  const rows = [];
  const shouldSpread = selectedWorkouts.length <= 4;

  selectedWorkouts.forEach((workout, index) => {
    const rawDay = Number(workout.day_number || index + 1);
    const day = shouldSpread ? (spreadWorkoutDays[index] || Math.min(7, index + 1)) : Math.max(1, Math.min(7, rawDay));
    rows.push({ type: "workout", day, workout });
  });

  for (let day = 1; day <= 7; day += 1) {
    if (rows.some((row) => row.day === day)) continue;
    const template = restTemplates[day] || {
      kind: "Пауза",
      title: "Восстановительный день",
      duration: "15 мин",
      description: "Лёгкая активность, прогулка и контроль самочувствия. Не каждая дата недели должна быть тяжёлой тренировкой.",
    };
    rows.push({ type: "support", day, ...template, product: template.product ? products[0] || null : null });
  }
...
export default function ProgramDetailPage() {
  ...
  const weeklySchedule = buildWeeklySchedule(selectedWorkouts, products);
          ...
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6 md:p-8">
              ...
              <div className="mt-6 space-y-3">
                {weeklySchedule.length ? weeklySchedule.map((entry, index) => {
                  const workout = entry.workout;
                  const isWorkout = entry.type === "workout";
                  const product = entry.product;

                  return (
                    <div key={`${entry.day}-${workout?.id || index}`} className="rounded-[1.5rem] border border-[color:var(--stroke)] bg-[color:var(--bg)] p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <div className={`text-xs font-black uppercase tracking-wider ${isWorkout ? "text-emerald-700 dark:text-emerald-300" : "text-cyan-700 dark:text-cyan-300"}`}>
                            День {entry.day} · {weekDayNames[entry.day - 1]}
                          </div>
                          <h3 className="mt-1 text-xl font-black text-[color:var(--text)]">
                            {isWorkout ? (workout.title || workout.name || "Тренировка") : entry.title}
                          </h3>
                        </div>
                        <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold ${isWorkout ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-cyan-500/10 text-cyan-700 dark:text-cyan-300"}`}>
                          {isWorkout ? <Clock3 className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
                          {isWorkout ? `${workout.duration_minutes || workout.duration || 45} мин` : entry.duration}
                        </div>
                      </div>
                      <p className="mt-3 text-sm leading-6 text-[color:var(--muted)]">
                        {isWorkout ? (workout.description || "Выполни разминку, основную часть и спокойную заминку. Следи за техникой.") : entry.description}
                      </p>

                      {!isWorkout && product ? (
                        <Link href={`/shop/${product.id}`} className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/8 p-3 transition hover:border-emerald-500/40">
                          <div className="min-w-0">
                            <div className="text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-300">Рекомендация дня</div>
                            <div className="mt-1 line-clamp-1 font-bold text-[color:var(--text)]">{product.name}</div>
                          </div>
                          <ArrowRight className="h-4 w-4 shrink-0 text-emerald-700 dark:text-emerald-300" />
                        </Link>
                      ) : null}
                    </div>
                  );
            ...
            <div className="rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6 md:p-7">
              ...
              <div className="mt-5 space-y-3">
                {!progress ? (
                  ...
                  <>
                    <button type="button" onClick={() => updateProgress(progress.status === "paused" ? "resume" : "advance")} disabled={Boolean(actionLoading)} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 py-3.5 font-black text-white transition hover:bg-emerald-600 disabled:opacity-60">
                      {actionLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <RotateCcw className="h-5 w-5" />} {progress.status === "paused" ? "Продолжить" : "Отметить неделю"}
                    </button>
