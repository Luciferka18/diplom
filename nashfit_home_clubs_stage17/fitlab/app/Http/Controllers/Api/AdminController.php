...
class AdminController extends Controller
{
    ...
    {
        $data = $request->validate(['status' => ['required', 'in:new,created,awaiting_payment,paid,processing,shipped,completed,cancelled,refunded']]);
    ...
    private function payload(Request $request, string $entity, bool $partial = false): array
    {
        return match ($entity) {
            ...
            'locations' => $request->validate([
                ...
                'address' => [$partial ? 'sometimes' : 'required', 'string', 'max:255'],
                'description' => ['nullable', 'string'],
