...
function StatCard({ icon: Icon, label, value, hint, href }) {
  const body = (
    <div className="group h-full rounded-[2rem] border border-stone-200 bg-white/95 p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-500/30 hover:shadow-md dark:border-white/10 dark:bg-white/[0.04]">
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300">
          <Icon className="h-5 w-5" />
        </div>
        {href ? <span className="text-xs font-bold text-emerald-700 opacity-0 transition group-hover:opacity-100 dark:text-emerald-300">Открыть</span> : null}
      </div>
      <div className="mt-5 text-3xl font-black leading-none tracking-tight text-slate-950 dark:text-white">{value}</div>
      <div className="mt-2 text-sm font-semibold text-slate-700 dark:text-white/75">{label}</div>
      {hint ? <div className="mt-1 text-xs font-medium text-slate-500 dark:text-white/45">{hint}</div> : null}
    </div>
  );
...
const quickActions = [
  { href: "/admin/orders", label: "Заказы", icon: ShoppingCart, primary: true },
...
function QuickActions() {
  return (
    <section className="rounded-[2rem] border border-stone-200 bg-white/95 p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.04] md:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950 dark:text-white">Быстрые действия</h2>
            <p className="text-sm font-medium text-slate-500 dark:text-white/55">Основные разделы админки без прокрутки вниз.</p>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={action.primary
                  ? "inline-flex items-center gap-2 rounded-full bg-emerald-700 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-emerald-800 dark:bg-emerald-500 dark:text-slate-950 dark:hover:bg-emerald-400"
                  : "inline-flex items-center gap-2 rounded-full border border-stone-200 bg-white px-5 py-3 text-sm font-black text-slate-950 shadow-sm transition hover:border-emerald-500/40 hover:bg-emerald-50 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
                }
              >
