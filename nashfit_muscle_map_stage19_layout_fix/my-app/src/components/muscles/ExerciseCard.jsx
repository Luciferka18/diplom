...
import { AlertTriangle, Dumbbell, PlayCircle, ShieldCheck } from "lucide-react";
...
export default function ExerciseCard({ exercise }) {
  ...
  const mistakes = exercise.common_mistakes || "Не делайте рывки, не сокращайте амплитуду и не теряйте контроль в негативной фазе.";
  ...
  return (
    <article className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-3.5 transition hover:-translate-y-0.5 hover:border-[#ff4d4f]/50">
      <div className="flex items-start justify-between gap-3">
        ...
        {exercise.video_url ? (
          <a href={exercise.video_url} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#ff4d4f] text-white" aria-label={`Открыть видео: ${exercise.name}`}>
            <PlayCircle size={19} />
          </a>
        ) : (
      ...
      <div className="mt-3 grid gap-2 text-xs leading-5 text-[color:var(--muted)]">
        ...
        <div className="rounded-xl bg-[#ff4d4f]/5 px-3 py-2">
          <span className="inline-flex items-center gap-1 font-bold text-[#c52b32]"><AlertTriangle size={13} /> Ошибка:</span> <span className="line-clamp-1">{mistakes}</span>
        </div>
      </div>

      <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-[color:var(--muted)]">
        <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">{exercise.equipment || "без оборудования"}</span>
        {primary ? <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">осн.: {primary}</span> : null}
        {secondary ? <span className="rounded-full border border-[color:var(--stroke)] bg-[color:var(--bg)] px-2.5 py-1">доп.: {secondary}</span> : null}
      </div>
    </article>
  );
}
