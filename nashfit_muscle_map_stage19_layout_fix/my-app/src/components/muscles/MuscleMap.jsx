        ...
        <div className="rounded-[1.9rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-3.5 shadow-[0_18px_60px_rgba(0,0,0,.06)] md:p-5">
          <div className="grid gap-5 xl:grid-cols-[minmax(0,610px)_minmax(360px,1fr)] xl:items-start">
            <div>
              ...
              <div className="mt-4 rounded-[1.35rem] border border-[color:var(--stroke)] bg-[color:var(--bg)] p-3">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div className="text-xs font-black uppercase tracking-[0.15em] text-[color:var(--muted)]">Быстрый выбор мышцы</div>
                  <div className="hidden text-xs font-bold text-[color:var(--muted)] sm:block">выберите область без точного попадания по SVG</div>
                </div>
                  ...
                  {chipList.map((muscle) => {
                    ...
                    return (
                      <button
                        ...
                        className={`min-h-[40px] w-full rounded-xl border px-2.5 py-2 text-center text-xs font-black leading-4 transition ${active ? "border-[#ff4d4f] bg-[#ff4d4f] text-white shadow-[0_10px_22px_rgba(255,77,79,.22)]" : "border-[color:var(--stroke)] bg-[color:var(--panel)] text-[color:var(--muted)] hover:border-[#ff4d4f]/50 hover:text-[color:var(--text)]"}`}
                      >
                        {muscle.name}
                      </button>
                    );
