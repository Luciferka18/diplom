...
export default function AdminProductsPage() {
  ...
  function edit(item){setEditing(item.id);setForm(toForm(item));setMessage("");window.scrollTo({top:0,behavior:"smooth"});}
  ...
  async function save(event){
    ...
    try{
      const payload={
        ...
        category_id:form.category_id?Number(form.category_id):null,is_featured:!!form.is_featured,is_new:!!form.is_new,trainer_pick:!!form.trainer_pick,home_use:!!form.home_use,is_active:!!form.is_active,
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={save} className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-6">
      ...
      <div className="mt-7 border-t border-[color:var(--stroke)] pt-6"><div className="flex items-center justify-between"><div><h3 className="flex items-center gap-2 text-lg font-black text-[color:var(--text)]"><Layers3 className="h-5 w-5 text-cyan-400" /> Варианты товара</h3><p className="text-sm text-[color:var(--muted)]">Например вкус + объём или размер + цвет.</p></div><button type="button" onClick={addVariant} className="inline-flex items-center gap-2 rounded-xl border border-cyan-400/30 px-4 py-2 font-bold text-cyan-300"><Plus className="h-4 w-4" /> Добавить вариант</button></div>
