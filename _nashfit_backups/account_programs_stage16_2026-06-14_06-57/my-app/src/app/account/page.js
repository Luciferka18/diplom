...
function TodayPanel({ user, upcomingBooking, latestOrder }) {
  const date = new Date();
  const isTrainer = user?.role === "trainer";
  const todayItems = [
    {
      icon: Dumbbell,
      label: isTrainer ? "Фокус дня" : "Тренировка",
      value: isTrainer ? "Клиенты и расписание" : "Верх тела + кор",
      tone: "emerald",
    },
    {
      icon: Flame,
      label: "Серия",
      value: isTrainer ? "5 активных дней" : "4 дня подряд",
      tone: "amber",
    },
    {
      icon: Target,
      label: "Цель",
      value: isTrainer ? "Закрыть заявки" : "8 000 шагов",
      tone: "cyan",
    },
  ];
    ...
    <section className="rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[color:var(--muted)]">Короткий план дня и важные события аккаунта.</p>
        </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        {todayItems.map((item) => (
          <div key={item.label} className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4">
            ...
            <p className="mt-3 text-xs font-bold uppercase tracking-wide text-[color:var(--muted)]">{item.label}</p>
            <p className="mt-1 font-black text-[color:var(--text)]">{item.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-[color:var(--muted)]">
            {upcomingBooking
              ? `${upcomingBooking.service?.name || upcomingBooking.title || "Тренировка"} — ${formatDate(getBookingDate(upcomingBooking))} ${formatTime(getBookingDate(upcomingBooking))}`
              : "Записей пока нет. Можно выбрать тренера и удобное время."}
          </p>
        </div>
        <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4">
          <div className="flex items-center gap-2 text-sm font-bold text-[color:var(--text)]">
          ...
          <p className="mt-2 text-sm leading-6 text-[color:var(--muted)]">
            {latestOrder
              ? `Заказ #${latestOrder.id} · ${getOrderTotal(latestOrder)} · ${latestOrder.status_label || latestOrder.status || "в обработке"}`
              : "История заказов появится после первой покупки в магазине."}
          </p>
        </div>
      </div>
    </section>
            ...
            <Link key={item.id} href={item.action_url || "/account/notifications"} className="block rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 transition hover:border-emerald-500/35">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 line-clamp-2 text-sm leading-6 text-[color:var(--muted)]">{item.body || item.message}</p>
                </div>
              </div>
            </Link>
          ))}
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const [ordersResponse, bookingsResponse, reviewsResponse, notificationsResponse] = await Promise.allSettled([
        apiGet("/orders"),
        apiGet("/bookings"),
        apiGet("/reviews"),
        apiGet("/account/notifications?per_page=6"),
      ]);
      ...
      setDashboard({ orders, bookings, reviews, notifications, unreadCount });
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6 md:p-8">
        ...
        <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/account/security" className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-sm font-black text-[color:var(--text)] transition hover:border-emerald-500/35">
              Безопасность
            </Link>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-6">
          <TodayPanel user={user} upcomingBooking={upcomingBooking} latestOrder={latestOrder} />
          ...
          <section className="rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[color:var(--muted)]">Демо-виджет активности</p>
              </div>
            </div>
            <div className="mt-5 space-y-4">
              {[
                ...
                <div key={label}>
                  <div className="mb-2 flex items-center justify-between text-sm font-bold text-[color:var(--text)]"><span>{label}</span><span>{value}%</span></div>
