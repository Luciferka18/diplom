  ...
  return (
    <main className="overflow-hidden">
      ...
      <section className="px-4 pb-8 md:px-8 md:pb-14">
        <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-7 text-[color:var(--text)] md:p-10 dark:bg-[#07101e] dark:text-white">
          <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              <div className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-300">НашФит рядом</div>
              <h2 className="mt-3 text-3xl font-black md:text-4xl">Попробуйте зал без риска: 3 посещения бесплатно</h2>
              <div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm text-[color:var(--muted)] dark:text-white/70">
                <span className="inline-flex items-center gap-2"><MapPin size={17} className="text-emerald-600 dark:text-emerald-300" /> {data.gym?.address || "ул. Спортивная, 10"}</span>
                <span className="inline-flex items-center gap-2"><Clock3 size={17} className="text-emerald-600 dark:text-emerald-300" /> {data.gym?.working_hours || "Ежедневно, 07:00–23:00"}</span>
              </div>
            </div>
            <Link href="/memberships" className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-4 font-black text-white hover:bg-emerald-700 dark:bg-emerald-400 dark:text-[#07101e] dark:hover:bg-emerald-300">Получить предложение <ArrowRight size={19} /></Link>
          </div>
        </div>
      </section>
