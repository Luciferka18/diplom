...
class NashfitDemoContentSeeder extends Seeder
{
    ...
    private function seedTags(): array
    {
        ...
        $result = [];
        foreach ($items as $slug => $name) $result[$slug] = $this->upsert(Tag::class, ['slug'=>$slug], ['name'=>$name]);
        return $result;
    }
    ...
    private function seedProducts(array $tags, array $trainers): array
    {
        ...
        $categories = [];
        foreach ($categoryData as $slug=>$name) $categories[$slug] = $this->upsert(Category::class,['slug'=>$slug],['name'=>$name]);
        $items = [
