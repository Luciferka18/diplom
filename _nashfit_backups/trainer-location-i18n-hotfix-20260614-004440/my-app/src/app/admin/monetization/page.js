...
const input = "w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-3 py-2.5 text-sm text-[color:var(--text)] outline-none focus:border-[color:var(--accent)]";
...
export default function MonetizationAdminPage() {
  ...
  return (
    <div className="space-y-6">
      <div><span className="text-xs font-bold uppercase tracking-[.2em] text-[color:var(--accent)]">Монетизация</span><h1 className="mt-2 text-3xl font-black text-[color:var(--text)]">Абонементы, акции и услуги</h1><p className="mt-1 text-[color:var(--muted)]">Единая база для mock-оплаты и будущего подключения реального провайдера.</p></div>
      <div className="grid grid-cols-2 gap-2 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-2 lg:grid-cols-4">{Object.entries(configs).map(([key, value]) => { const Icon = value.icon; return <button key={key} onClick={() => setEntity(key)} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-sm font-bold ${entity === key ? "bg-[color:var(--accent)] text-[color:var(--on-accent)]" : "text-[color:var(--muted)] hover:bg-[color:var(--bg)]"}`}><Icon className="h-4 w-4" />{value.title}</button>; })}</div>

      {message ? <div className="rounded-xl border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-4 text-[color:var(--accent)]">{message}</div> : null}
      ...
      <form onSubmit={save} className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-7">
        ...
        <button disabled={busy} className="mt-6 flex items-center gap-2 rounded-xl bg-[color:var(--accent)] px-5 py-3 font-black text-[color:var(--on-accent)] disabled:opacity-60">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : editing ? <Save className="h-5 w-5" /> : <Plus className="h-5 w-5" />}{busy ? "Сохраняем…" : editing ? "Сохранить" : "Создать"}</button>
      </form>
      ...
      <section className="overflow-hidden rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)]">
        ...
        {loading ? <div className="flex items-center gap-2 p-6 text-[color:var(--muted)]"><Loader2 className="h-5 w-5 animate-spin" />Загрузка…</div> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-[color:var(--bg)] text-xs uppercase tracking-wider text-[color:var(--muted)]"><tr>{config.columns.map((k) => <th key={k} className="px-5 py-3">{k.replaceAll("_", " ")}</th>)}<th className="px-5 py-3 text-right">Действия</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t border-[color:var(--stroke)]"><td className="px-5 py-4 font-bold text-[color:var(--text)]">{cell(item, config.columns[0])}</td>{config.columns.slice(1).map((k) => <td key={k} className="px-5 py-4 text-[color:var(--muted)]">{cell(item, k)}</td>)}<td className="px-5 py-4"><div className="flex justify-end gap-2"><button onClick={() => edit(item)} className="rounded-lg border border-[color:var(--stroke)] p-2 text-[color:var(--secondary)]"><Pencil className="h-4 w-4" /></button><button onClick={() => remove(item.id)} className="rounded-lg border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] p-2 text-[color:var(--danger)]"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table>{items.length === 0 ? <div className="p-8 text-center text-[color:var(--muted)]">Пока нет записей.</div> : null}</div>}
      </section>
    </div>
  );
}
