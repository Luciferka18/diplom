...
function TrainerContent({ params, router }) {
  ...
  const goToBooking = () => {
    ...
    if (selectedSlot.location_id) {
      params.set("locationId", String(selectedSlot.location_id));
    }
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
        <div className="space-y-6">
            ...
            <Card hover={false}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {availableSlots.map((slot) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-1 rounded-lg border px-2 py-2 text-xs font-medium transition ${selectedSlot?.time === slot.time && selectedSlot?.schedule_id === slot.schedule_id ? "border-[color:var(--accent)] bg-[color:var(--accent)] text-[color:var(--on-accent)]" : "border-[color:var(--stroke)] bg-[color:var(--panel)] text-[color:var(--text)] hover:border-[color:var(--accent)] hover:text-[color:var(--accent)]"}`}
                    >
              ...
              {selectedSlot ? (
                <div className="mt-4 rounded-xl border border-[color:var(--accent)]/30 bg-[color:var(--accent)]/10 p-4">
                  <p className="text-sm text-[color:var(--muted)]">Вы выбрали <b className="text-[color:var(--text)]">{selectedSlot.time}</b>. Переход к оформлению произойдёт только после подтверждения.</p>
                  <button onClick={goToBooking} className="mt-3 w-full rounded-xl bg-[color:var(--accent)] px-4 py-3 font-bold text-[color:var(--on-accent)] transition hover:opacity-90">
                    Записаться на выбранное время
                  </button>
                </div>
              ) : availableSlots.length > 0 && !slotsLoading ? (
                <p className="mt-3 text-xs text-[color:var(--muted)]">Выберите время, затем подтвердите запись отдельной кнопкой.</p>
              ) : null}
            </Card>
          ) : null}
          ...
          <Card hover={false}>
            ...
            <div className="mt-5 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-4 text-sm leading-6 text-[color:var(--muted)]">
              Чтобы записаться, выберите рабочий день и свободное время в расписании выше.
            </div>
          </Card>
        </div>
