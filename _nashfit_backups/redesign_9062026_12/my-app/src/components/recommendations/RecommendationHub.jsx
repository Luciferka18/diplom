...
const CONTEXT_COPY = {
  ...
  account: {
    ...
    productsTitle: "Подобрано под ваши цели",
    actionsTitle: "Что поможет двигаться дальше",
  },
};
...
function ProductRecommendation({ item }) {
  ...
  return (
    <Link
      ...
      className="group flex min-h-[190px] overflow-hidden rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--bg)] transition duration-300 hover:-translate-y-1 hover:border-emerald-400/50 hover:shadow-[0_18px_55px_rgba(16,185,129,0.08)]"
    >
      <div className="relative w-[38%] min-w-[112px] overflow-hidden bg-gradient-to-br from-emerald-500/20 via-cyan-500/10 to-slate-950">
        {item.image_url ? (
          <img
            src={item.image_url}
            alt={item.name}
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-3 text-center">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-200/75">
              {category}
            </span>
          </div>
        )}
        ...
        {item.badge ? (
          <span className="absolute left-2 top-2 rounded-full border border-white/10 bg-slate-950/80 px-2.5 py-1 text-[10px] font-black text-emerald-300 backdrop-blur">
            {item.badge}
          </span>
        ) : null}
      </div>
        ...
        <div className="mt-auto pt-4">
          {Number(item.rating || 0) > 0 ? (
            <div className="mb-2 flex items-center gap-1 text-xs text-[color:var(--muted)]">
              ...
              {item.reviews_count ? <span>· {item.reviews_count} отзывов</span> : null}
            </div>
          ) : null}
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              {item.old_price ? (
                <div className="text-[11px] text-[color:var(--muted)] line-through">
                  {rubles(item.old_price)}
                </div>
              ) : null}
            </div>
...
function TrainerRecommendation({ item }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-3xl border border-cyan-400/25 bg-gradient-to-br from-cyan-500/10 via-cyan-500/5 to-transparent p-5">
      <div className="flex items-start gap-4">
        <div className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-cyan-300/20 bg-cyan-400/10">
          {item.image_url ? (
            ...
            <div className="flex h-full items-center justify-center text-xl font-black text-cyan-200">
              {initial}
            </div>
          )}
        </div>
        <div className="min-w-0">
          <div className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300">
          ...
          <p className="mt-1 line-clamp-1 text-sm text-cyan-200/80">
      ...
      <div className="mt-auto flex flex-wrap gap-2 pt-5">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-4 py-2 text-sm font-black text-slate-950 transition hover:bg-cyan-300"
        >
...
function MembershipRecommendation({ item }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-3xl border border-violet-400/25 bg-gradient-to-br from-violet-500/20 via-violet-500/5 to-transparent p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">
      ...
      {features.length ? (
        <ul className="mt-4 space-y-2 text-xs text-violet-100/80">
          {features.map((feature) => (
            <li key={feature} className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-violet-300" /> {feature}
            </li>
          ))}
        </ul>
      ) : null}
      ...
      <div className="mt-auto flex items-end justify-between gap-4 pt-5">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-violet-400 px-4 py-2.5 text-sm font-black text-slate-950 transition hover:bg-violet-300"
        >
      ...
      <div className="border-b border-[color:var(--stroke)] bg-gradient-to-r from-emerald-500/10 via-cyan-500/5 to-transparent px-5 py-6 sm:px-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-400">
          ...
          {data.reason ? (
            <div className="max-w-md rounded-2xl border border-emerald-400/20 bg-emerald-400/5 px-4 py-3 text-xs leading-5 text-emerald-100/75">
              <span className="font-black text-emerald-300">Почему это вам:</span> {data.reason}
            </div>
          ) : null}
        </div>
      </div>
          ...
          <div>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <h3 className="inline-flex items-center gap-2 text-lg font-black text-[color:var(--text)]">
                <Package className="h-5 w-5 text-emerald-400" /> {copy.productsTitle}
              </h3>
              <Link href="/shop" className="text-sm font-bold text-emerald-400 transition hover:text-emerald-300">
                Перейти в магазин →
              </Link>
            </div>
        ...
        {actions.length ? (
          <div>
            <h3 className="mb-4 inline-flex items-center gap-2 text-lg font-black text-[color:var(--text)]">
              <Dumbbell className="h-5 w-5 text-cyan-300" /> {copy.actionsTitle}
            </h3>
            <div className="grid grid-cols-[repeat(auto-fit,minmax(285px,1fr))] gap-4">
              {actions.map((item) =>
