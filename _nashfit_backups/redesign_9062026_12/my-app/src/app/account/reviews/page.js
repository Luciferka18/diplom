...
function StarsView({ value = 0, size = "md" }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star key={star} className={`${sizeClass} ${star <= safeValue ? "fill-yellow-400 text-yellow-400" : "text-[color:var(--stroke)]"}`} />
      ))}
    </div>
  );
}
    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button" onClick={() => onChange(star)} onMouseEnter={() => setHover(star)} onMouseLeave={() => setHover(0)} className="transition-transform hover:scale-110" aria-label={`${star} из 5`}>
          <Star className={`w-8 h-8 ${star <= (hover || value) ? "fill-yellow-400 text-yellow-400" : "text-[color:var(--stroke)]"}`} />
        </button>
      ))}
    </div>
  );
...
export default function AccountReviewsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {successMessage ? <Card hover={false} className="p-4 bg-emerald-500/10 border-emerald-500/30"><div className="flex items-center gap-3"><CheckCircle2 className="w-5 h-5 text-emerald-400 flex-shrink-0" /><p className="text-emerald-300">{successMessage}</p></div></Card> : null}
      {error ? <Card hover={false} className="p-4 bg-red-500/10 border-red-500/30"><div className="flex items-center gap-3"><AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" /><p className="text-red-300">{error}</p></div></Card> : null}

      <Card hover={false} className="p-6">
        <div className="flex items-center gap-2 mb-4"><MessageSquare className="w-5 h-5 text-emerald-400" /><h2 className="text-lg font-semibold text-white">Написать отзыв</h2></div>
          ...
          {reviews.map((review) => (
            <Card key={review.id} hover={false} className="p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                ...
                <Badge className={review.status === "approved" ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/30" : "bg-yellow-500/20 text-yellow-300 border-yellow-500/30"}>{review.status === "approved" ? "Опубликован" : "На модерации"}</Badge>
              </div>
              <p className="text-[color:var(--text)] whitespace-pre-wrap">{review.text}</p>
              {review.response ? <div className="mt-4 p-4 rounded-xl bg-[color:var(--panel)] border border-[color:var(--stroke)]"><p className="text-sm font-medium text-emerald-400 mb-1">Ответ администрации:</p><p className="text-[color:var(--text)]">{review.response}</p></div> : null}
            </Card>
          ))}
        </div>
      </div>
