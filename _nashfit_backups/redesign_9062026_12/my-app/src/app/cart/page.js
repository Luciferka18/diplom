  ...
  if (!cart.length) return (
    <main className="container-fitlab py-16">
      <div className="mx-auto max-w-2xl rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-10 text-center">
        <div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-emerald-400/10"><ShoppingBag className="h-9 w-9 text-emerald-400" /></div>
        ...
        <Link href="/shop" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-6 py-3 font-black text-slate-950">Перейти в магазин <ArrowRight className="h-5 w-5" /></Link>
      </div>
    </main>
  );
  ...
  return (
    <main className="min-h-screen bg-[color:var(--bg)] pb-20 pt-10">
      <div className="container-fitlab">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.18em] text-emerald-400">Покупки</div><h1 className="mt-1 text-4xl font-black text-[color:var(--text)]">Корзина</h1><p className="mt-2 text-[color:var(--muted)]">{totalCount} товаров на сумму {money(totalPrice)}</p></div><button onClick={() => window.confirm("Очистить корзину?") && clearCart()} className="inline-flex items-center gap-2 text-sm font-semibold text-[color:var(--muted)] hover:text-red-400"><Trash2 className="h-4 w-4" /> Очистить</button></div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_390px]">
          <section className="space-y-4">
            ...
            <Link href="/shop" className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--stroke)] px-5 py-3 font-bold text-[color:var(--text)] hover:border-emerald-400/40">← Продолжить покупки</Link>
          </section>
          ...
          <aside className="h-fit lg:sticky lg:top-24">
            <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6">
              ...
              <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-4">
                <div className="flex items-center gap-2 text-sm font-bold text-emerald-300"><Truck className="h-4 w-4" /> {remaining > 0 ? `До бесплатной доставки ${money(remaining)}` : "Доставка будет бесплатной"}</div>
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-950/40"><div className="h-full rounded-full bg-emerald-400 transition-all" style={{ width: `${progress}%` }} /></div>
              </div>

              <div className="mt-5"><label className="text-xs font-black uppercase tracking-wider text-[color:var(--muted)]">Промокод</label><div className="mt-2 flex gap-2"><input value={promo} onChange={(e) => setPromo(e.target.value.toUpperCase())} placeholder="START10" className="min-w-0 flex-1 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 font-bold uppercase text-[color:var(--text)] outline-none focus:border-emerald-400" /><button onClick={applyPromo} disabled={!promo.trim() || promoBusy} className="rounded-xl border border-emerald-400/30 px-4 font-bold text-emerald-400 disabled:opacity-40">Применить</button></div>{promoMessage ? <div className="mt-2 text-xs text-emerald-400">{promoMessage}</div> : null}{promoError ? <div className="mt-2 text-xs text-red-300">{promoError}</div> : null}</div>

              <div className="mt-6 space-y-3 border-t border-[color:var(--stroke)] pt-5 text-sm"><div className="flex justify-between text-[color:var(--muted)]"><span>Товары</span><b className="text-[color:var(--text)]">{money(totalPrice)}</b></div>{discount > 0 ? <div className="flex justify-between text-emerald-400"><span>Скидка</span><b>−{money(discount)}</b></div> : null}<div className="flex justify-between text-[color:var(--muted)]"><span>Доставка</span><b className="text-[color:var(--text)]">Рассчитаем при оформлении</b></div><div className="flex justify-between border-t border-[color:var(--stroke)] pt-4 text-xl"><span className="font-black text-[color:var(--text)]">Итого</span><b className="text-emerald-400">{money(totalAfterDiscount)}</b></div></div>
              <Link href="/checkout" className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 px-5 py-3.5 font-black text-slate-950 hover:bg-emerald-300">Перейти к оформлению <ArrowRight className="h-5 w-5" /></Link>
      ...
      {recommendations.length ? <section className="container-fitlab mt-14"><div className="text-xs font-black uppercase tracking-wider text-cyan-400">Рекомендации тренеров</div><h2 className="mt-1 text-3xl font-black text-[color:var(--text)]">Добавить к заказу</h2><div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{recommendations.map((product) => <ProductCard key={product.id} product={product} compact />)}</div></section> : null}
    </main>
  );
}
