  ...
  return (
    <main className="min-h-screen bg-[color:var(--bg)] py-12">
      <div className="container-fitlab">
        <section className="relative overflow-hidden rounded-[2rem] border border-emerald-400/20 bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950/60 px-6 py-12 md:px-12 md:py-16">
          <div className="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-emerald-400/15 blur-3xl" />
          <div className="relative max-w-3xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-emerald-300"><Sparkles className="h-4 w-4" /> Абонементы НашФит</span>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {promotions.slice(0, 2).map((item) => (
              <div key={item.id} className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-5">
                <div className="flex items-center gap-2 text-sm font-bold text-cyan-200"><TicketPercent className="h-5 w-5" /> {item.badge || "Акция"}</div>
                <div className="mt-2 text-xl font-bold text-white">{item.banner_title || item.name}</div>
                <p className="mt-1 text-sm text-white/60">{item.banner_text || item.description}</p>
              </div>
            ))}
          </div>
        ) : null}
        ...
        <div className="mt-10 flex flex-col gap-4 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex w-full max-w-md gap-2">
            <input value={promo} onChange={(e) => setPromo(e.target.value.toUpperCase())} placeholder="START10" className="min-w-0 flex-1 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 font-semibold uppercase text-[color:var(--text)] outline-none focus:border-emerald-400" />
          ...
          {loading ? Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-96 animate-pulse rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)]" />) : plans.map((plan) => {
            ...
            return (
              <article key={plan.id} className={`relative flex min-h-[420px] flex-col overflow-hidden rounded-3xl border p-6 ${plan.is_featured ? "border-emerald-400/50 bg-gradient-to-b from-emerald-400/15 to-[color:var(--panel)] shadow-xl shadow-emerald-950/20" : "border-[color:var(--stroke)] bg-[color:var(--panel)]"}`}>
                ...
                {plan.is_trial ? (
                  <div className="mt-6 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-center text-sm font-semibold text-emerald-300">Выдаётся автоматически после регистрации</div>
                ) : isAuthed ? (
