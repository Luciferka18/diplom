...
export default function ArticlePage() {
  ...
  return (
    <main className="pb-20">
      <div className="fixed left-0 top-0 z-[70] h-1 bg-[color:var(--accent)] transition-[width]" style={{ width: `${progress}%` }} />

      <Container size="wide" className="py-7">
        <Link href="/articles" className="inline-flex items-center gap-2 text-sm font-semibold text-[color:var(--muted)] hover:text-[color:var(--accent-hover)]">
          ...
          <div className="overflow-hidden rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)]">
            ...
            <div className="p-6 sm:p-10 lg:p-14">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full bg-[color:var(--accent-soft)] px-3 py-1.5 text-sm font-bold text-[color:var(--accent)]">{categoryLabel(article.category)}</span>
                {trainer ? <span className="inline-flex items-center gap-1.5 rounded-full border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] px-3 py-1.5 text-sm font-bold text-[color:var(--accent)]"><BadgeCheck className="h-4 w-4" /> Материал тренера</span> : null}
              </div>
              ...
              <div className="mt-8 flex flex-col gap-5 border-t border-[color:var(--stroke)] pt-6 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-emerald-400 to-cyan-400 text-lg font-black text-[color:var(--on-accent)]">
        ...
        <div className="grid gap-8 lg:grid-cols-[220px_minmax(0,760px)_220px] lg:justify-center">
            ...
            {prepared.toc.length ? (
              <div className="sticky top-24">
                ...
                <nav className="mt-4 space-y-3 border-l border-[color:var(--stroke)] pl-4">
                  {prepared.toc.map((item) => <a key={item.id} href={`#${item.id}`} className={`block text-sm leading-5 text-[color:var(--muted)] hover:text-[color:var(--accent-hover)] ${item.level === "h3" ? "pl-3" : "font-semibold"}`}>{item.title}</a>)}
                </nav>
              </div>
            ) : null}
          </aside>
          ...
          <aside>
            <div className="sticky top-24 space-y-3">
              <button type="button" onClick={toggleFavorite} className={`flex w-full items-center gap-3 rounded-2xl border p-4 text-left text-sm font-semibold transition ${article.is_favorited ? "border-[color:var(--accent)] bg-[color:var(--accent-soft)] text-[color:var(--accent)]" : "border-[color:var(--stroke)] bg-[color:var(--panel)] hover:border-[color:var(--accent-border)]"}`}>
                <Bookmark className={`h-5 w-5 ${article.is_favorited ? "fill-emerald-400" : ""}`} /> {article.is_favorited ? "Сохранено" : "Сохранить"}
              </button>
              <button type="button" onClick={copyLink} className="flex w-full items-center gap-3 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-4 text-left text-sm font-semibold hover:border-[color:var(--accent-border)]">
                {copied ? <CheckCircle2 className="h-5 w-5 text-[color:var(--accent)]" /> : <Copy className="h-5 w-5" />} {copied ? "Скопировано" : "Поделиться"}
              </button>
            </div>
          </aside>
        </div>
      ...
      <Container size="narrow" className="mt-12">
        ...
        <Link
          href="/programs"
          className="group mt-8 flex items-center gap-4 rounded-2xl border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-4 transition hover:border-[color:var(--accent-border)] hover:bg-[color:var(--accent-soft)]"
        >
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[color:var(--accent-soft)]">
            <Dumbbell className="h-5 w-5 text-[color:var(--accent)]" />
          </div>
          ...
          <ArrowRight className="h-5 w-5 shrink-0 text-[color:var(--accent)] transition group-hover:translate-x-1" />
        </Link>
