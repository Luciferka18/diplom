                    ...
                    {config.columns.map((column) => {
                      ...
                      return (
                        <td key={column.key} className={`px-4 py-3 ${column.truncate ? "max-w-[320px] truncate" : ""}`} title={column.truncate ? String(display) : undefined}>
                          {column.badge ? <StatusBadge value={display} /> : String(display)}
                        </td>
                      );
                    })}
...
function StatusBadge({ value }) {
  ...
  const className = status.includes("paid") || status.includes("published") || status.includes("completed")
    ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-500"
    : status.includes("cancel") || status.includes("refund")
