...
class TrainerScheduleController extends Controller
{
    ...
    public function availableSlots($trainerId, Request $request)
    {
        ...
        $service = $request->filled('service_id') ? TrainerService::find($request->integer('service_id')) : null;
        if ($service && $service->trainer_id && (int) $service->trainer_id !== (int) $trainerId) {
            return response()->json(['message' => 'Услуга недоступна у этого тренера.'], 422);
        }
        ...
        foreach ($schedules as $schedule) {
            $durationMinutes = (int) ($service?->duration_minutes ?: $schedule->slot_duration_minutes ?: 60);
