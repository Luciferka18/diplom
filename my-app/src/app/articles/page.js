        ...
        <Container size="wide">
          <div className="grid items-end gap-8 lg:grid-cols-[1fr_auto]">
            <div className="max-w-3xl">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] px-3 py-1.5 text-sm font-bold text-[color:var(--accent)]">
      ...
      <Container size="wide" className="py-10">
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_300px]">
          <div className="min-w-0">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                <label className="relative flex-1">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] pl-12 pr-4 outline-none transition focus:border-[color:var(--accent)] focus:ring-2 focus:ring-emerald-400/20"
                  />
                </label>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`inline-flex h-12 items-center gap-2 rounded-2xl border px-4 text-sm font-semibold transition ${trainerOnly ? "border-[color:var(--accent)] bg-[color:var(--accent-soft)] text-[color:var(--accent)]" : "border-[color:var(--stroke)] text-[color:var(--muted)] hover:text-[color:var(--text)]"}`}
                  >
              ...
              <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                {ARTICLE_CATEGORIES.map((item) => (
                  <button
                    ...
                    className={`shrink-0 rounded-full border px-4 py-2 text-sm font-semibold transition ${category === item.value ? "border-[color:var(--accent)] bg-[color:var(--accent)] text-[color:var(--on-accent)]" : "border-[color:var(--stroke)] text-[color:var(--muted)] hover:border-[color:var(--accent-border)] hover:text-[color:var(--text)]"}`}
                  >
                    {item.label}
                  </button>
                ))}
            ...
            {visibleFeatured ? (
              <div className="mt-8">
                <div className="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-[.16em] text-[color:var(--accent)]">
            ...
            {loading ? (
              ...
              <div className="mt-8 rounded-2xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] bg-[color:var(--danger-soft)] p-5 text-[color:var(--danger)]">{error}</div>
          ...
          <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
              ...
              <div className="mt-5 space-y-4">
                {popular.map((article, index) => (
                  <Link key={article.id} href={`/articles/${article.id}`} className="group grid grid-cols-[34px_1fr] gap-3">
                    <span className="text-2xl font-black text-[color:var(--accent)]/50">{String(index + 1).padStart(2, "0")}</span>
                    <div>
                      <p className="line-clamp-2 text-sm font-bold leading-5 transition group-hover:text-[color:var(--accent-hover)]">{article.title}</p>
            ...
            <div className="overflow-hidden rounded-3xl border border-[color:var(--accent-border)] bg-gradient-to-br from-emerald-500/20 to-cyan-500/10 p-6">
              <Dumbbell className="h-9 w-9 text-[color:var(--accent)]" />
