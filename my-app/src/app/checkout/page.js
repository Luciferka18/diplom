      ...
      <form onSubmit={submit} className="container-fitlab">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_390px]">
          <section className="space-y-5">
            <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[color:var(--accent)] font-black text-[color:var(--on-accent)]">1</span><div><h2 className="text-xl font-black text-[color:var(--text)]">Контакты</h2><p className="text-sm text-[color:var(--muted)]">Для подтверждения заказа</p></div></div><div className="mt-5 grid gap-4 md:grid-cols-2"><input value={form.customer_name} onChange={(e) => setForm({...form,customer_name:e.target.value})} placeholder="Имя получателя" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-[color:var(--accent)]" /><input value={form.customer_phone} onChange={(e) => setForm({...form,customer_phone:e.target.value})} placeholder="Телефон" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-[color:var(--accent)]" /><input value={form.customer_email} onChange={(e) => setForm({...form,customer_email:e.target.value})} placeholder="Email" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-[color:var(--accent)] md:col-span-2" /></div></div>

            <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[color:var(--accent)] font-black text-[color:var(--on-accent)]">2</span><div><h2 className="text-xl font-black text-[color:var(--text)]">Способ получения</h2><p className="text-sm text-[color:var(--muted)]">Самовывоз или курьер</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setDelivery("pickup")} className={`rounded-2xl border p-5 text-left ${delivery === "pickup" ? "border-[color:var(--accent)] bg-[color:var(--accent-soft)]" : "border-[color:var(--stroke)]"}`}><Store className="h-6 w-6 text-[color:var(--accent)]" /><div className="mt-3 font-black text-[color:var(--text)]">Самовывоз</div><div className="mt-1 text-sm text-[color:var(--muted)]">Бесплатно из клуба «НашФит»</div></button><button type="button" onClick={() => setDelivery("courier")} className={`rounded-2xl border p-5 text-left ${delivery === "courier" ? "border-[color:var(--accent)] bg-[color:var(--accent-soft)]" : "border-[color:var(--stroke)]"}`}><Truck className="h-6 w-6 text-[color:var(--secondary)]" /><div className="mt-3 font-black text-[color:var(--text)]">Курьер</div><div className="mt-1 text-sm text-[color:var(--muted)]">390 ₽ или бесплатно от 5 000 ₽</div></button></div>
              {delivery === "pickup" ? <div className="mt-4 rounded-xl border border-[color:var(--stroke)] p-4 text-sm text-[color:var(--muted)]"><b className="text-[color:var(--text)]">Фитнес-клуб «НашФит»</b><br />Заберите заказ после уведомления о готовности.</div> : <div className="mt-5"><div className="mb-3 flex flex-wrap gap-2">{addresses.map((address) => <button type="button" key={address.id} onClick={() => chooseAddress(address)} className={`rounded-full border px-3 py-1.5 text-sm ${selectedAddress === address.id ? "border-[color:var(--accent)] bg-[color:var(--accent)] text-[color:var(--on-accent)]" : "border-[color:var(--stroke)] text-[color:var(--muted)]"}`}><MapPin className="mr-1 inline h-3.5 w-3.5" />{address.label}</button>)}</div><div className="grid gap-4 md:grid-cols-2"><input value={form.city} onChange={(e) => setForm({...form,city:e.target.value})} placeholder="Город" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none" /><input value={form.postal_code} onChange={(e) => setForm({...form,postal_code:e.target.value})} placeholder="Индекс" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none" /><input value={form.address_line} onChange={(e) => setForm({...form,address_line:e.target.value})} placeholder="Улица, дом, квартира" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none md:col-span-2" /></div>{!selectedAddress ? <label className="mt-4 flex cursor-pointer items-center gap-3 text-sm text-[color:var(--muted)]"><input type="checkbox" checked={saveAddress} onChange={(e) => setSaveAddress(e.target.checked)} className="h-4 w-4 accent-emerald-400" /> Сохранить адрес для следующих заказов</label> : null}</div>}
            </div>

            <div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[color:var(--accent)] font-black text-[color:var(--on-accent)]">3</span><div><h2 className="text-xl font-black text-[color:var(--text)]">Комментарий</h2><p className="text-sm text-[color:var(--muted)]">Необязательно</p></div></div><textarea value={form.comment} onChange={(e) => setForm({...form,comment:e.target.value})} placeholder="Например, позвонить за час до доставки" className="mt-5 min-h-28 w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 text-[color:var(--text)] outline-none focus:border-[color:var(--accent)]" /></div>
          </section>

          <aside className="h-fit lg:sticky lg:top-24"><div className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-6"><h2 className="text-xl font-black text-[color:var(--text)]">Итого</h2><div className="mt-5 max-h-60 space-y-3 overflow-auto pr-1">{cart.map((item) => <div key={item.key} className="flex gap-3"><div className="h-14 w-14 overflow-hidden rounded-xl bg-slate-900">{item.image_url ? <img src={item.image_url} alt="" className="h-full w-full object-cover" /> : null}</div><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold text-[color:var(--text)]">{item.name}</div><div className="text-xs text-[color:var(--muted)]">{item.variant?.name ? `${item.variant.name} · ` : ""}{item.quantity} шт.</div></div><b className="text-sm text-[color:var(--text)]">{money(item.price * item.quantity)}</b></div>)}</div><div className="mt-5 border-t border-[color:var(--stroke)] pt-5"><label className="text-xs font-black uppercase tracking-wider text-[color:var(--muted)]"><TicketPercent className="mr-1 inline h-4 w-4" /> Промокод</label><div className="mt-2 flex gap-2"><input value={promo} onChange={(e) => setPromo(e.target.value.toUpperCase())} className="min-w-0 flex-1 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-3 py-2.5 font-bold uppercase text-[color:var(--text)] outline-none" /><button type="button" onClick={() => validatePromo()} className="rounded-xl border border-[color:var(--accent-border)] px-3 font-bold text-[color:var(--accent)]"><Check className="h-4 w-4" /></button></div></div><div className="mt-5 space-y-3 text-sm"><div className="flex justify-between text-[color:var(--muted)]"><span>Товары</span><b className="text-[color:var(--text)]">{money(totalPrice)}</b></div>{discount ? <div className="flex justify-between text-[color:var(--accent)]"><span>Скидка</span><b>−{money(discount)}</b></div> : null}<div className="flex justify-between text-[color:var(--muted)]"><span>Доставка</span><b className="text-[color:var(--text)]">{deliveryCost ? money(deliveryCost) : "Бесплатно"}</b></div><div className="flex justify-between border-t border-[color:var(--stroke)] pt-4 text-xl"><span className="font-black text-[color:var(--text)]">К оплате</span><b className="text-[color:var(--accent)]">{money(total)}</b></div></div><button disabled={busy} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[color:var(--accent)] px-5 py-3.5 font-black text-[color:var(--on-accent)] disabled:opacity-50"><ShieldCheck className="h-5 w-5" />{busy ? "Создаём заказ…" : "Оформить и оплатить"}</button><p className="mt-3 text-center text-xs text-[color:var(--muted)]">Оплата работает в демонстрационном режиме</p></div></aside>
        </div>
      </form>
