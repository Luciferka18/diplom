...
export default function AdminProductsPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.18em] text-[color:var(--accent)]">Управление магазином</div><h1 className="mt-1 text-3xl font-black text-[color:var(--text)]">Товары и варианты</h1><p className="mt-2 text-[color:var(--muted)]">Цены, остатки, вкусы, размеры и подборки.</p></div><button onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--stroke)] px-4 py-2.5 font-bold text-[color:var(--text)]"><RefreshCw className="h-4 w-4" /> Обновить</button></div>
    {error?<div className="rounded-xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] bg-[color:var(--danger-soft)] p-4 text-[color:var(--danger)]">{error}</div>:null}{message?<div className="rounded-xl border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-4 text-[color:var(--accent)]">{message}</div>:null}
    ...
    <form onSubmit={save} className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-6">
      ...
      <div className="mt-4 flex flex-wrap gap-2">{[["is_featured","На главной"],["is_new","Новинка"],["trainer_pick","Выбор тренера"],["home_use","Для дома"],["is_active","Активен"]].map(([key,label])=><label key={key} className={`cursor-pointer rounded-full border px-3 py-2 text-sm font-bold ${form[key]?"border-[color:var(--accent)] bg-[color:var(--accent)] text-[color:var(--on-accent)]":"border-[color:var(--stroke)] text-[color:var(--muted)]"}`}><input type="checkbox" checked={!!form[key]} onChange={e=>setForm({...form,[key]:e.target.checked})} className="hidden" />{label}</label>)}</div>

      <div className="mt-7 border-t border-[color:var(--stroke)] pt-6"><div className="flex items-center justify-between"><div><h3 className="flex items-center gap-2 text-lg font-black text-[color:var(--text)]"><Layers3 className="h-5 w-5 text-[color:var(--secondary)]" /> Варианты товара</h3><p className="text-sm text-[color:var(--muted)]">Например вкус + объём или размер + цвет. Итоговый остаток товара считается по вариантам автоматически.</p></div><button type="button" onClick={addVariant} className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--secondary-border)] px-4 py-2 font-bold text-[color:var(--secondary)]"><Plus className="h-4 w-4" /> Добавить вариант</button></div>
        <div className="mt-4 space-y-3">{form.variants.map((variant,index)=><div key={variant.id||index} className="grid gap-3 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 md:grid-cols-2 xl:grid-cols-[1.2fr_1fr_1.5fr_.7fr_.7fr_auto]"><input required value={variant.name} onChange={e=>updateVariant(index,"name",e.target.value)} placeholder="Название варианта" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] px-3 py-2.5 text-[color:var(--text)]" /><input required value={variant.sku} onChange={e=>updateVariant(index,"sku",e.target.value)} placeholder="SKU" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] px-3 py-2.5 text-[color:var(--text)]" /><input value={variant.options_text||""} onChange={e=>updateVariant(index,"options_text",e.target.value)} placeholder="Вкус: Шоколад, Объём: 900 г" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] px-3 py-2.5 text-[color:var(--text)]" /><input type="number" step="0.01" value={variant.price??""} onChange={e=>updateVariant(index,"price",e.target.value)} placeholder="Цена" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] px-3 py-2.5 text-[color:var(--text)]" /><input type="number" value={variant.stock??0} onChange={e=>updateVariant(index,"stock",e.target.value)} placeholder="Остаток" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] px-3 py-2.5 text-[color:var(--text)]" /><button type="button" onClick={()=>setForm(f=>({...f,variants:f.variants.filter((_,i)=>i!==index)}))} className="grid h-11 w-11 place-items-center rounded-xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] text-[color:var(--danger)]"><Trash2 className="h-4 w-4" /></button></div>)}</div>
      </div>
      <div className="mt-6 flex gap-3"><button disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-[color:var(--accent)] px-5 py-3 font-black text-[color:var(--on-accent)] disabled:opacity-50"><Save className="h-5 w-5" />{busy?"Сохраняем…":editing?"Сохранить изменения":"Создать товар"}</button>{editing?<button type="button" onClick={reset} className="rounded-xl border border-[color:var(--stroke)] px-5 py-3 font-bold text-[color:var(--text)]">Отмена</button>:null}</div>
    </form>
    ...
    <section className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-[color:var(--text)]">Каталог</h2><p className="text-sm text-[color:var(--muted)]">{products.length} товаров</p></div><label className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[color:var(--muted)]" /><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Поиск" className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] py-2.5 pl-10 pr-4 text-[color:var(--text)]" /></label></div>
      <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[800px] text-left text-sm"><thead className="text-[color:var(--muted)]"><tr><th className="pb-3">Товар</th><th className="pb-3">Цена</th><th className="pb-3">Остаток</th><th className="pb-3">Варианты</th><th className="pb-3">Статус</th><th></th></tr></thead><tbody>{filtered.map(item=><tr key={item.id} className="border-t border-[color:var(--stroke)]"><td className="py-4"><div className="flex items-center gap-3">{item.image_url?<img src={item.image_url} alt="" className="h-12 w-12 rounded-xl object-cover"/>:<div className="grid h-12 w-12 place-items-center rounded-xl bg-[color:var(--accent-soft)]"><Package className="h-5 w-5 text-[color:var(--accent)]"/></div>}<div><b className="text-[color:var(--text)]">{item.name}</b><div className="text-xs text-[color:var(--muted)]">{item.brand}</div></div></div></td><td className="py-4 font-bold text-[color:var(--text)]">{Number(item.price).toLocaleString("ru-RU")} ₽</td><td className="py-4 text-[color:var(--text)]">{item.stock}</td><td className="py-4 text-[color:var(--muted)]">{item.variants?.length||0}</td><td className="py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.is_active?"bg-[color:var(--accent-soft)] text-[color:var(--accent)]":"bg-[color:var(--danger-soft)] text-[color:var(--danger)]"}`}>{item.is_active?"Активен":"Скрыт"}</span></td><td className="py-4"><div className="flex justify-end gap-2"><button onClick={()=>edit(item)} className="rounded-lg border border-[color:var(--stroke)] px-3 py-2 font-bold text-[color:var(--text)]">Изменить</button><button onClick={()=>remove(item)} className="rounded-lg border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] p-2 text-[color:var(--danger)]"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table></div>
    </section>
  </div>;
}
