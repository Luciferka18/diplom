...
const inputClass = "w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-3 py-2.5 text-sm text-[color:var(--text)] outline-none focus:border-[color:var(--accent)]";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.18em] text-[color:var(--accent)]">Контекстная монетизация</p>
      ...
      <Card hover={false} className="border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-5">
        <div className="flex items-start gap-3">
          <WandSparkles className="mt-0.5 h-6 w-6 shrink-0 text-[color:var(--accent)]" />
          <div>
      ...
      {message ? <div className="rounded-2xl border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-4 text-[color:var(--accent)]">{message}</div> : null}
      ...
      <Card hover={false} className="overflow-hidden p-0">
        ...
        {loading ? (
          <div className="flex min-h-52 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[color:var(--accent)]" /></div>
        ) : items.length === 0 ? (
          <div className="py-14 text-center"><Sparkles className="mx-auto h-9 w-9 text-[color:var(--accent)]" /><p className="mt-3 font-black">Всё работает автоматически</p><p className="mt-1 text-sm text-[color:var(--muted)]">Ручных переопределений пока нет.</p></div>
        ) : (
          <div className="divide-y divide-[color:var(--stroke)]">
            {items.map((item) => (
              <div key={item.id} className="grid gap-4 p-5 lg:grid-cols-[1fr_1fr_auto] lg:items-center">
                <div>
                  <div className="text-xs font-black uppercase tracking-wider text-[color:var(--accent)]">{sourceTypes.find(([value]) => value === item.source_type)?.[1]}</div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => edit(item)} className="rounded-xl border border-[color:var(--stroke)] p-2 text-[color:var(--secondary)]"><Pencil className="h-4 w-4" /></button>
                  <button type="button" onClick={() => remove(item)} className="rounded-xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] p-2 text-[color:var(--danger)]"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            ))}
          </div>
