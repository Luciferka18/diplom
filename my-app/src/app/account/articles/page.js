  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-bold uppercase tracking-[.15em] text-[color:var(--accent)]">Авторский кабинет</p>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card hover={false} className="flex items-center gap-4 p-4">
          <div className="rounded-xl bg-[color:var(--accent-soft)] p-3 text-[color:var(--accent)]"><BookOpen className="h-5 w-5" /></div>
        ...
        <Card hover={false} className="flex items-center gap-4 p-4">
          <div className="rounded-xl bg-[color:var(--warning-soft)] p-3 text-[color:var(--warning)]"><FileText className="h-5 w-5" /></div>
      ...
      {loading ? (
        <Card hover={false} className="flex min-h-52 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[color:var(--accent)]" /></Card>
      ) : items.length === 0 ? (
                ...
                <div className="p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex shrink-0 gap-2">
                      ...
                      <Button size="sm" variant="ghost" className="text-[color:var(--danger)]" onClick={() => removeArticle(article)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                  {article.status === "rejected" && article.rejection_reason ? (
                    <div className="mt-4 rounded-xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] bg-[color:var(--danger-soft)] p-3 text-sm text-[color:var(--danger)]"><strong>Причина отклонения:</strong> {article.rejection_reason}</div>
                  ) : null}
