  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="h-64 animate-pulse rounded-3xl bg-[color:var(--panel)]" /> : active ? (
        <section className="overflow-hidden rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-7 md:p-9 dark:border-emerald-400/30 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950/50">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-start">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300"><CheckCircle2 className="h-4 w-4" /> Активен</span>
              <h2 className="mt-4 text-3xl font-black text-[color:var(--text)] dark:text-white">{active.membership?.name}</h2>
              <p className="mt-2 max-w-2xl text-[color:var(--muted)] dark:text-white/60">{active.membership?.description}</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-5 text-right dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-wider text-[color:var(--muted)] dark:text-white/45">Действует до</div>
              <div className="mt-1 text-xl font-bold text-[color:var(--text)] dark:text-white">{date(active.ends_at)}</div>
            </div>
          </div>
          <div className="mt-7 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 dark:border-white/10 dark:bg-white/5"><CalendarDays className="h-5 w-5 text-emerald-600 dark:text-emerald-300" /><div className="mt-3 text-xs text-[color:var(--muted)] dark:text-white/45">Начало</div><div className="font-bold text-[color:var(--text)] dark:text-white">{date(active.starts_at)}</div></div>
            <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 dark:border-white/10 dark:bg-white/5"><Clock3 className="h-5 w-5 text-cyan-600 dark:text-cyan-300" /><div className="mt-3 text-xs text-[color:var(--muted)] dark:text-white/45">Срок</div><div className="font-bold text-[color:var(--text)] dark:text-white">{active.membership?.duration_months || 1} мес.</div></div>
            <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 dark:border-white/10 dark:bg-white/5"><CreditCard className="h-5 w-5 text-amber-600 dark:text-violet-300" /><div className="mt-3 text-xs text-[color:var(--muted)] dark:text-white/45">Оплачено</div><div className="font-bold text-[color:var(--text)] dark:text-white">{money(active.total_amount)}</div></div>
          </div>
        </section>
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.id} className="flex flex-col gap-4 rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-3"><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === "active" ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-yellow-500/10 text-yellow-700 dark:text-yellow-200"}`}>{item.status === "active" ? "Активен" : "Ожидает оплаты"}</span>{item.payment?.status === "pending" ? <button onClick={() => setPayment(item.payment)} className="rounded-xl bg-emerald-400 px-4 py-2 text-sm font-bold text-slate-950">Оплатить</button> : null}</div>
            </div>
          ))}
        </div>
      </section>
