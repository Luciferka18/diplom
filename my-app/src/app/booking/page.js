    ...
    calendarCells.push(
      <button
        ...
        className={`relative min-h-12 rounded-xl border text-sm font-bold transition ${active
          ...
          : availability
            ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300 hover:border-emerald-400 hover:bg-emerald-400/20"
      ...
      >
        {day}
        {availability && !active ? <span className="absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-emerald-400" /> : null}
      </button>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[color:var(--bg)] py-10">
      <div className="container-fitlab max-w-6xl">
        <section className="overflow-hidden rounded-[2rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] p-7 md:p-10 dark:border-cyan-400/20 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-cyan-950/50">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-300"><Dumbbell className="h-4 w-4" /> Офлайн-тренировки</span>
              <h1 className="mt-4 text-4xl font-black text-[color:var(--text)] md:text-5xl dark:text-white">Тренировка, которая учитывает именно вашу цель</h1>
              <p className="mt-4 max-w-2xl leading-7 text-[color:var(--muted)] dark:text-white/60">Выберите формат, свободный день и время. Выбор слота ничего не оформляет — запись создастся только после нажатия отдельной кнопки.</p>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 dark:border-white/10 dark:bg-white/5"><UserRound className="h-5 w-5 text-cyan-600 dark:text-cyan-300" /><div className="mt-3 font-bold text-[color:var(--text)] dark:text-white">Личный тренер</div><div className="mt-1 text-xs text-[color:var(--muted)] dark:text-white/50">Подбор под вашу цель</div></div>
              <div className="rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-4 dark:border-white/10 dark:bg-white/5"><CalendarDays className="h-5 w-5 text-emerald-600 dark:text-emerald-300" /><div className="mt-3 font-bold text-[color:var(--text)] dark:text-white">Свободные дни</div><div className="mt-1 text-xs text-[color:var(--muted)] dark:text-white/50">Календарь обновляется автоматически</div></div>
            </div>
          </div>
        </section>

        {!isAuthed ? <div className="mt-6 rounded-2xl border border-yellow-500/25 bg-yellow-500/10 p-4 text-sm text-yellow-700 dark:text-yellow-100">Для записи нужно <Link href="/login?next=/booking" className="font-bold underline">войти в аккаунт</Link>.</div> : null}
        ...
        <form onSubmit={submit} className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            <section className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-7">
              <div className="mb-5 flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-400 font-black text-slate-950">1</span><div><h2 className="font-bold text-[color:var(--text)]">Тренер и формат</h2><p className="text-sm text-[color:var(--muted)]">Выберите специалиста и тип занятия</p></div></div>
              <select value={form.trainer_id} onChange={(event) => change("trainer_id", event.target.value)} disabled={loading} className="w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-emerald-400" required>
            ...
            <section className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-7">
              <div className="mb-5 flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-400 font-black text-slate-950">2</span><div><h2 className="font-bold text-[color:var(--text)]">Свободный день и время</h2><p className="text-sm text-[color:var(--muted)]">Активны только дни, в которые у тренера остались места</p></div></div>
            ...
            <section className="rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 md:p-7">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[color:var(--muted)]">Имя</label><input value={form.client_name} onChange={(event) => change("client_name", event.target.value)} placeholder="Как к вам обращаться" className="w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-emerald-400" required /></div>
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[color:var(--muted)]">Телефон</label><div className="relative"><Phone className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[color:var(--muted)]" /><input value={form.client_phone} onChange={(event) => change("client_phone", event.target.value)} placeholder="+7 999 000-00-00" className="w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] py-3 pl-11 pr-4 text-[color:var(--text)] outline-none focus:border-emerald-400" required /></div></div>
              </div>
              <textarea value={form.client_comment} onChange={(event) => change("client_comment", event.target.value)} placeholder="Цель тренировок, ограничения, пожелания…" className="mt-4 min-h-28 w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 text-[color:var(--text)] outline-none focus:border-emerald-400" />
            </section>
          </div>
          ...
          <aside className="h-fit rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-5 lg:sticky lg:top-24">
            ...
            <div className="mt-5 space-y-4 text-sm">
              ...
              <div className="flex gap-3"><CalendarDays className="h-5 w-5 shrink-0 text-violet-400" /><div><div className="text-[color:var(--muted)]">Когда</div><div className="font-bold text-[color:var(--text)]">{form.date && form.time ? `${new Date(`${form.date}T12:00:00`).toLocaleDateString("ru-RU")} в ${form.time}` : "Не выбрано"}</div></div></div>
            ...
            <input value={form.promo_code} onChange={(event) => change("promo_code", event.target.value.toUpperCase())} placeholder="START10" className="mt-2 w-full rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] px-4 py-3 font-bold uppercase text-[color:var(--text)] outline-none focus:border-emerald-400" />
            ...
            <button type="submit" disabled={!isAuthed || busy || !form.time} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 px-5 py-3 font-black text-slate-950 hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-40">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <ArrowRight className="h-5 w-5" />}{busy ? "Создаём запись…" : service?.price ? "Записаться и перейти к оплате" : "Записаться бесплатно"}</button>
