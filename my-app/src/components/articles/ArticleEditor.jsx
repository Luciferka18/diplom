...
export default function ArticleEditor({ initialArticle = null }) {
  ...
  return (
    <div className="space-y-5">
      {restorableDraft ? (
        <div className="flex flex-col gap-3 rounded-2xl border border-[color:color-mix(in_srgb,var(--warning)_40%,var(--stroke))] bg-[color:var(--warning-soft)] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
      ...
      {error ? <div className="rounded-2xl border border-[color:color-mix(in_srgb,var(--danger)_40%,var(--stroke))] bg-[color:var(--danger-soft)] p-4 text-sm text-[color:var(--danger)]">{error}</div> : null}
      {message ? <div className="rounded-2xl border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] p-4 text-sm text-[color:var(--accent)]">{message}</div> : null}
        ...
        <div className="space-y-4">
          ...
          <div className={`mx-auto overflow-hidden rounded-3xl border border-[color:var(--stroke)] bg-[color:var(--panel)] shadow-2xl transition-all ${previewDevice === "mobile" ? "max-w-[390px]" : "max-w-5xl"}`}>
            ...
            <div className="p-6 sm:p-10">
              <span className="text-sm font-bold uppercase tracking-[.16em] text-[color:var(--accent)]">{categoryLabel(category)}</span>
              ...
              <div className="sticky top-0 z-10 flex flex-wrap items-center gap-1 border-b border-[color:var(--stroke)] bg-[color:var(--panel)]/95 p-2 backdrop-blur">
                {toolbar.map((item, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="rounded-lg p-2 text-[color:var(--muted)] transition hover:bg-[color:var(--accent-soft)] hover:text-[color:var(--accent-hover)]"
                    >
                ...
                <div className="group relative ml-auto">
                  ...
                  <div className="invisible absolute right-0 top-full z-20 mt-1 w-52 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--panel)] p-2 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100">
                    <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => addCallout("tip")} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[color:var(--accent-soft)]">Совет тренера</button>
                    <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => addCallout("important")} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[color:var(--warning-soft)]">Важно</button>
                    <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => addCallout("mistake")} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[color:var(--danger-soft)]">Частая ошибка</button>
                  </div>
                </div>
              </div>
          ...
          <aside className="space-y-5 xl:sticky xl:top-5 xl:self-start">
            <Card hover={false}>
              ...
              <div className="mt-5">
                ...
                <button
                  ...
                  className="mt-2 flex aspect-[16/10] w-full items-center justify-center overflow-hidden rounded-2xl border border-dashed border-[color:var(--stroke)] bg-[color:var(--bg)] text-center transition hover:border-[color:var(--accent)]"
                >
                  {coverImage ? (
                ...
                {coverImage ? (
                  <button type="button" onClick={() => setCoverImage("")} className="mt-2 inline-flex items-center gap-1 text-xs text-[color:var(--danger)] hover:text-[color:var(--danger)]">
              ...
              <div className="mt-5 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg)] p-3 text-sm text-[color:var(--muted)]">
                {isAdmin || isTrainer ? (
                  <><strong className="text-[color:var(--accent)]">Публикация без модерации.</strong> После нажатия «Опубликовать» статья сразу появится в журнале.</>
                ) : (
                  <><strong className="text-[color:var(--warning)]">Нужна модерация.</strong> Администратор проверит материал перед публикацией.</>
                )}
              </div>
            </Card>
            ...
            <Card hover={false}>
              ...
              <ul className="mt-3 space-y-2 text-sm text-[color:var(--muted)]">
                <li className="flex gap-2"><Check className={`mt-0.5 h-4 w-4 ${title.length >= 5 ? "text-[color:var(--accent)]" : "text-[color:var(--muted2)]"}`} /> Понятный заголовок</li>
                <li className="flex gap-2"><Check className={`mt-0.5 h-4 w-4 ${excerpt.length >= 40 ? "text-[color:var(--accent)]" : "text-[color:var(--muted2)]"}`} /> Короткое описание</li>
                <li className="flex gap-2"><Check className={`mt-0.5 h-4 w-4 ${wordCount >= 80 ? "text-[color:var(--accent)]" : "text-[color:var(--muted2)]"}`} /> Полезный основной текст</li>
                <li className="flex gap-2"><Check className={`mt-0.5 h-4 w-4 ${coverImage ? "text-[color:var(--accent)]" : "text-[color:var(--muted2)]"}`} /> Обложка</li>
              </ul>
            </Card>
          </aside>
        </div>
