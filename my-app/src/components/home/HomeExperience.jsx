...
const programImageFallbacks = {
  "функциональныи-старт": "/seed-images/programs/funktsionalnyy-start.webp",
  "похудение-за-8-недель": "/seed-images/programs/pokhudenie-za-8-nedel.webp",
  "силовая-база": "/seed-images/programs/silovaya-baza.webp",
  "мощь-и-сила": "/seed-images/programs/moshch-i-sila.webp",
  "иога-для-начинающих": "/seed-images/programs/yoga-dlya-nachinayushchikh.webp",
  "стретчинг-плюс": "/seed-images/programs/stretching-plus.webp",
  "funktsionalnyy-start": "/seed-images/programs/funktsionalnyy-start.webp",
  "pokhudenie-za-8-nedel": "/seed-images/programs/pokhudenie-za-8-nedel.webp",
  "silovaya-baza": "/seed-images/programs/silovaya-baza.webp",
  "moshch-i-sila": "/seed-images/programs/moshch-i-sila.webp",
  "yoga-dlya-nachinayushchikh": "/seed-images/programs/yoga-dlya-nachinayushchikh.webp",
  "stretching-plus": "/seed-images/programs/stretching-plus.webp"
};

const articleImageFallbacks = {
  "kak-nachat-trenirovatsya-rukovodstvo-dlya-novichkov": "/seed-images/articles/kak-nachat-trenirovatsya.webp",
  "pitanie-dlya-nabora-myshechnoy-massy": "/seed-images/articles/pitanie-dlya-massy.webp",
  "yoga-dlya-nachinayushchikh-pervye-shagi": "/seed-images/articles/yoga-dlya-nachinayushchikh.webp",
  "5-oshibok-pri-pokhudenii": "/seed-images/articles/oshibki-pri-pokhudenii.webp",
  "vosstanovlenie-posle-trenirovki": "/seed-images/articles/vosstanovlenie-posle-trenirovki.webp",
  "как-начать-тренироваться-руководство-для-новичков": "/seed-images/articles/kak-nachat-trenirovatsya.webp",
  "питание-для-набора-мышечнои-массы": "/seed-images/articles/pitanie-dlya-massy.webp",
  "иога-для-начинающих-первые-шаги": "/seed-images/articles/yoga-dlya-nachinayushchikh.webp",
  "5-ошибок-при-похудении": "/seed-images/articles/oshibki-pri-pokhudenii.webp",
  "восстановление-после-тренировки": "/seed-images/articles/vosstanovlenie-posle-trenirovki.webp"
};

function imageFor(kind, item, explicit) {
  if (explicit) return explicit;

  const keys = [item?.slug, item?.name, item?.title, item?.id].map(normalizeKey).filter(Boolean);
  const map = kind === "trainer"
    ? trainerImageFallbacks
    : kind === "product"
      ? productImageFallbacks
      : kind === "program"
        ? programImageFallbacks
        : articleImageFallbacks;
...
function ProgramCard({ item }) {
  return (
    <Link href={`/programs/${item.id}`} className="group overflow-hidden rounded-[1.6rem] border border-[color:var(--stroke)] bg-[color:var(--panel)] transition hover:-translate-y-1 hover:border-[color:var(--accent)]/45 hover:shadow-xl hover:shadow-emerald-950/10 dark:hover:shadow-black/30">
      <Media src={imageFor("program", item, item.image_url || item.cover_image_url)} alt={item.title} fallback="/seed-images/programs/funktsionalnyy-start.webp" className="aspect-[16/10]" />
      <div className="p-5">
...
function TrainerCard({ item, featured = false }) {
  return (
    ...
    >
      <Media
        src={imageFor("trainer", item, item.photo_url || item.image_url || item.avatar_url)}
        alt={item.name}
        fallback="/seed-images/trainers/anna-kuznetsova.webp"
...
function ProductCard({ item, featured = false }) {
  return (
    ...
    >
      <Media src={imageFor("product", item, item.image_url || item.gallery?.[0] || item.variants?.find?.((variant) => variant?.image_url)?.image_url)} alt={item.name} fallback="/seed-images/products/whey-protein-gold-standard/main.webp" className={`${featured ? "aspect-[1/1] md:h-full" : "h-28 w-28 aspect-square"} rounded-[1.35rem] border border-[color:var(--stroke)]`} />
      <div className="min-w-0 flex flex-col">
        <div className="flex flex-wrap items-center gap-2">
...
function ArticleCard({ item, featured = false }) {
  return (
    ...
    >
      <Media src={imageFor("article", item, item.cover_image_url || item.image_url)} alt={item.title} fallback="/seed-images/articles/kak-nachat-trenirovatsya.webp" className={`${featured ? "aspect-[4/5] md:h-full" : "h-36 aspect-[4/3]"} rounded-[1.35rem] border border-[color:var(--stroke)]`} />
      <div className="min-w-0 flex flex-col">
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[color:var(--accentGlow)] px-2.5 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-[color:var(--accent)]">
