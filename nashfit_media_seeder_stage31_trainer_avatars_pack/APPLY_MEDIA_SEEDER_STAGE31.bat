...
if not exist "%PROJECT_ROOT%\my-app" (
  ...
  echo Распакуйте nashfit_media_seeder_stage31_trainer_avatars_pack в корень проекта, где лежат my-app и fitlab.
  pause
  exit /b 1
)
...
if not exist "%BACKUP_DIR%\my-app\src\components\articles" mkdir "%BACKUP_DIR%\my-app\src\components\articles"
...
echo.
echo [OK] Media Seeder Stage31 установлен.
echo.
echo Что появилось:
...
echo - avatar_url теперь отдаётся в ArticleResource, ReviewResource, BookingResource и TrainerResource
echo - отзывы на главной показывают аватары пользователей
echo.
