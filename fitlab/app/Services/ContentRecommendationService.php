...
class ContentRecommendationService
{
    public function forArticle(Article $article, ?User $user = null): array
    {
        ...
        $products = $this->productsByCategories($productCategories, 2);
        $trainers = collect();
        if ($article->author?->trainerProfile) {
            $trainers->push($article->author->trainerProfile);
        }
        $trainers = $trainers->merge($this->topTrainers(1))->unique('id')->take(1)->values();
    ...
    public function forProgram(Program $program, ?User $user = null): array
    {
        ...
        return $this->compose(
            ...
            $this->productsByCategories($categories, 3),
            $trainers,
            $this->featuredMemberships(1)
        );
    }
    ...
    public function forAccount(User $user): array
    {
        ...
        $products = $this->availableProducts()
            ...
            ->limit(12)
            ->get()
            ->unique(fn (Product $item) => Str::lower(trim($item->name)))
            ->take(3)
            ->values();
    ...
    {
        $normalizeName = fn (Product $item) => Str::lower(trim($item->name));
        ...
        $items = $this->availableProducts()
            ...
            ->limit($fetchLimit)
            ->get()
            ->unique($normalizeName)
            ->take($limit)
            ->values();

        if ($items->count() < $limit) {
            $existingNames = $items->map($normalizeName);
            $fallback = $this->availableProducts()
                ...
                ->limit($fetchLimit)
                ->get()
                ->reject(fn (Product $item) => $existingNames->contains($normalizeName($item)))
                ->unique($normalizeName)
